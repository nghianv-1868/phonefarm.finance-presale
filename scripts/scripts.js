...
const URL = process.env.TESTNET_0_URL;
...
const pointAddress = pointJson.networks[process.env.VUE_APP_CHAIN_ID_NETWORK].address;
const marketAddress = marketJson.networks[process.env.VUE_APP_CHAIN_ID_NETWORK].address;
...
exports.mintPoint = async function (oneAddress, amount) {
  try {
    ...
    point.wallet.addByPrivateKey(PRIVATE_KEY);
...
exports.createItem = async function (name, price, oneAddress, imageUrl, itemType) {
  try {
    ...
    market.wallet.addByPrivateKey(PRIVATE_KEY);
...
exports.buyItem = async function (itemId, price) {
  try {
    const market = hmy.contracts.createContract(marketJson.abi, marketAddress);
    market.wallet.addByPrivateKey(privateKey);
...
exports.setupItems = async function (items) {
  const address = hmy.crypto.getAddress(ADDRESS).checksum;
  const market = hmy.contracts.createContract(marketJson.abi, marketAddress);
  market.wallet.addByPrivateKey(PRIVATE_KEY);
...
exports.fetchPriceONE_USD = async function () {
  try {
    const market = hmy.contracts.createContract(marketJson.abi, marketAddress);
    let answer = await market.methods.getLatestPrice().call(options);
